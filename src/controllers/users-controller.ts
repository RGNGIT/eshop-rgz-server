...
class UserController {
  async register(req: Request, res: Response) {
    ...
    res.json({ token, fullName: `${newUser.last_name} ${newUser.name} ${newUser.middle_name}`, login: newUser.login, role: newUser.role, avatar: newUser.avatar });
  }
  ...
  async login(req: Request, res: Response) {
    ...
    if (user.password == foundYou.password) {
      ...
      res.json({ token, fullName: `${foundYou.last_name} ${foundYou.name} ${foundYou.middle_name}`, login: foundYou.login, role: foundYou.role, avatar: foundYou.avatar });
      return;
    }
